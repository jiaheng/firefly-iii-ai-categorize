...
- `OPENAI_WEB_SEARCH_OPTIONS` / `openai.web_search_options`: Web search options for enhanced categorization (optional). Must be a JSON object. Example: `{"user_location": {"country": "US"}}`
...
**Required fields:**
- `user_location.country`: ISO 3166-1 alpha-2 country code (e.g., "US", "GB", "CA", "DE")
...
```shell
docker run -d \
  -e OPENAI_WEB_SEARCH_OPTIONS='{"user_location": {"country": "US"}}' \
  ...
```

**Note:** When using `web_search_options`, ensure that `user_location.country` is provided, as it is required for this feature to work properly.
